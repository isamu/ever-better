...
**もう一つの始め方で、`freeze` の上に重ねるものではなく代替です。** 全ルールは error のまま、今日落ちている「ファイル×ルール」だけを生成ファイル `eslint-tier.config.js` で **warn** に降格します。そのファイルが挙げられている理由を直して再実行すると、そのエントリは消えます — **誰も設定を書き換えないままルールが厳しくなります**。
...
生成ファイルは `eslint-suppressions.json` と同じくツールが所有していて上書き安全です。あなたの config は**一度だけ**編集され、最後に spread されます（後勝ちのため）。

**生成される2つのファイルは両方コミットしてください。** ESLint が読むのは `eslint-tier.config.js`、次回の実行が比較する台帳は `.ever-better/tier.json` です。台帳が無ければ比較対象が無いので新しく tier を取り直します。そして**空の台帳は「無い」とは別物**です — リストが空になったリポジトリは最も厳しい状態であり、そこに現れた違反は他と全く同じように拒否されます。
