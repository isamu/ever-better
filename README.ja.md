...
CI では `ever-better tier` ではなく **`ever-better tier --check`** を回してください。ゲートが対象リポジトリを書き換えてはいけないからです。`--check` は**リポジトリの中には何も書きません** — スキャンはツリー外の一時ディレクトリで走るので、read-only の checkout でも動き、`git status` も汚しません。
