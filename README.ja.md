...
| P3 drain | 1ルールずつ潰す。見つけたバグはテストで固定 | 未実装 |
| P4 tighten | 次の階層のルールを追加して繰り返す | 未実装 |
| P5 split & DRY | 大きなファイルを分割し、重複を排除 | 未実装 |

価値が積み上がるのは P3〜P5 です。実装されるまでは `QUALITY.md` が残件の少ない順にリストを出すので、通常のエージェントセッションで進められます。
