...
**そして、生成される設定が認める免除はこれ「だけ」です。** `eslint-disable` コメントは無効化してあります（`noInlineConfig`）。`@ts-ignore` / `@ts-nocheck` / `as` / `!` / `any` も同様ですが、理由は厳しさではありません — disable コメントは違反を linter からも `eslint-suppressions.json` からも消してしまうからです。誰も数えず、`status` や `report` にも出ず、`prune` でも回収できない。「天井は下がることはあっても上がらない」という唯一の約束の外側にある、**永久の免除**になります。免除したいものは freeze してください。そこで初めて「下がるべき数字」になります。
...
npx ever-better secrets      # 履歴全体を走査して、コミットされた認証情報を探す
npx ever-better tier         # 全ルール error、引っかかるファイルだけ warn に降格
...
`--fan-in` は各行に数字をもう1つ足します。**そのファイルが何ファイルから import されているか**です。「どれくらい難しいか」のもう半分は「修正がどこまで届くか」だからです。20ファイルから import されているモジュールの `no-explicit-any` は export された型を変えることになり、エラーは diff が一度も開いていないファイルに出ます。
...
`gitleaks` で**履歴と作業ツリーの両方**を走査し、1件でも見つかれば失敗します。`bootstrap` は
`.github/workflows/secret-scan.yml` も書き出すので、こちらは push 前の確認用です。

両方を走査するのは、片方だけでは秘密情報を抱えたリポジトリが通ってしまうからです。履歴の走査は「1時間前に貼り付けてまだコミットしていない鍵」を見逃し、作業ツリーの走査は「コミットして後で消した鍵」——全員のクローンに残っているもの——を見逃します。コミットが1つも無いリポジトリでは、履歴の走査は何も読まずに `no leaks found` を返します。

**このツールで唯一「ベースラインを持たない」チェックで、そこが要点です。** 他のルールは全部「今あるものを記録して、そこから増やさない」ですが、**コミットされた鍵はもう公開済み**です — 全員のクローンに入っていて、どこかのダッシュボードにも載っています。免除する対象が存在せず、直し方はローテーションであってコミットではありません。出力もそう書いてあり、「その行を消せば直る」とは言いません。

答えは2つではなく3つです。gitleaks は指定しない限り「見つけた」と「自分が動けなかった」を同じ終了コードで返すからです:

| | |
| --- | --- |
| クリーン | exit 0 |
| 秘密情報を検出 | exit **2**、内容は redact して表示 |
| **走査できなかった** | exit **1**、その旨を明示 — クリーンではない |

最後の行が見た目より重要です。git の作業ツリー外で `gitleaks detect` を実行すると、エラーを出しつつ**0コミットを走査**して `no leaks found` で **exit 0** を返します — 何も読んでいない走査に対する「異常なし」です。ここではそれを通さず拒否します。

生成されるワークフローには、間違えやすい要点を入れてあります: `gitleaks-action`（GitHub Organization 配下ではライセンスキーが必要）ではなく MIT の CLI を使う、ダウンロードをチェックサム検証する、`fetch-depth: 0`（浅いクローンは漏洩したコミットを取りこぼす）、`--redact`（秘密情報を公開ログに残さない）。

### `tier`

```bash
ever-better tier            # tier を取り、台帳に記録する
ever-better tier --check    # 台帳と比較するだけ。何も書かない（CI 用）
```

**もう一つの始め方で、`freeze` の上に重ねるものではなく代替です。** 全ルールは error のまま、今日落ちている「ファイル×ルール」だけを生成ファイル `eslint-tier.config.mjs` で **warn** に降格します。そのファイルが挙げられている理由を直して再実行すると、そのエントリは消えます — **誰も設定を書き換えないままルールが厳しくなります**。

| | `freeze` | `tier` |
| --- | --- | --- |
| エディタ上 | 見えない（波線なし） | 編集中に警告として見える |
| `eslint .` | 沈黙 | 毎回出る |
| 粒度 | ファイル×ルール、件数つき | ファイル×ルール、件数つき |
| CI のゲート | `ever-better check` | `ever-better tier --check` |
| 締め方 | `prune`（違反単位） | 再実行（違反単位） |

**リストは縮むことしかできません。それを強制するのが `--check` です。** 免除されていないのに落ちるペアは「既に error だったルールを新しいコードが破った」ことです。そして**免除された件数より多く落ちるペア**も、姿を変えた同じものです — ペア全体が warn なので `eslint .` は exit 0 で何も気づきません。`.ever-better/tier.json` が件数を持つのはこのためで、増えていれば `tier` も `tier --check` も拒否します。

CI では `ever-better tier` ではなく **`ever-better tier --check`** を回してください。ゲートが対象リポジトリを書き換えてはいけないからで、`--check` は何も書きません。

**両方は動かさないでください。** warn に降格した違反は suppressions の台帳から warning の母集団へ**移動**するので、両方動かすと同じ違反が精密な台帳と粗い台帳の両方に載ります。

生成ファイルは `eslint-suppressions.json` と同じくツールが所有していて上書き安全です。あなたの config は**一度だけ**編集され、最後に spread されます（後勝ちのため）。 名前は `eslint-tier.config.mjs`、`eslint.config.cjs` の隣なら `eslint-tier.config.cjs` です — 素の `.js` にはしません（`"type": "module"` の無いパッケージでは CommonJS 扱いになるため）。
...
config の形によっては spread を自動で追加できません（配列を変数に束ねている場合など）。そのときは**「効いていない tier」を記録せずに拒否**し、貼り付ける2行を表示します。ESLint が探す6つの名前すべてに対応しています（`eslint.config.mts` / `.cts` を含む）。

生成ファイルは**自分自身を lint 対象から外します**。「編集するな」と書いてあるファイルの指摘は誰も直せませんし、実際に勝手に出ます — パスが長いと行が `printWidth` を超え、Prettier がエラーを出します。しかもそれはリストを作った scan の**後**に現れるので、何にも免除されません。古いバージョンが生成した名前を config が指したままなら、`tier` が指し先を張り替え、置き換えられた側のファイルを削除します。

**生成される2つのファイルは両方コミットしてください。** ESLint が読むのは `eslint-tier.config.mjs`、次回の実行が比較する台帳は `.ever-better/tier.json` です。台帳が無ければ比較対象が無いので新しく tier を取り直します。そして**空の台帳は「無い」とは別物**です — リストが空になったリポジトリは最も厳しい状態であり、そこに現れた違反は他と全く同じように拒否されます。
