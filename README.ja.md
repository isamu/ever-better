...
| `eslint .` | 沈黙 | 毎回出る |
| 粒度 | ファイル×ルール、**件数つき** | ファイル×ルール、件数なし |
| 締め方 | `prune`（違反単位） | 再実行（ファイル×ルール単位） |

歯止めは2つで、片方はタダで手に入ります。**warning の総数は既に ratchet 済み**です（`check` は `state.counters` も見るので、総数が baseline を超えれば今でも落ちます）。そして**リストは縮むことしかできません** — 免除されていないのに落ちるペアは「既に error だったルールを新しいコードが破った」ことなので、`tier` は書き込みを拒否して exit 1 になります。

`freeze` に対して失うのは**粒度**です（ルール別・ファイル別の件数）。既にリストにあるファイルは、総数だけに支えられて内部が腐りえます。
